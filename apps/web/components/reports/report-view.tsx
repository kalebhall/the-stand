...
export function ReportView({ slug, data }: { slug: string; data: ReportData }) {
  if (slug === 'speakers') {
    return (
      <ReportSection title="Speaker frequency" count={`${data.speakers.length} speakers`} empty="No recorded talks match filters.">
  ...
  if (slug === 'topics') {
    return (
      <ReportSection title="Topic history" count={`${data.topics.length} topics`} empty="No recorded speaker topics match filters.">
  ...
  if (slug === 'hymns') {
    return (
      <ReportSection title="Hymn frequency" count={`${data.hymns.length} hymns`} empty="No recorded hymns match filters.">
        {data.hymns.map((hymn) => (
          <li
  ...
  if (slug === 'prayers') {
    return (
      <ReportSection title="Prayer frequency" count={`${data.prayers.length} assignments`} empty="No recorded prayers match filters.">
  ...
  return (
    ...
    >
      {data.completeness.map((warning, index) => (
        <li
