...
export function validatePublicLayout(value: { preset: string; announcementMode: string; coverMode: string; coverImageUrl?: string; coverImageAltText?: string }): string | null {
  ...
  if (value.coverImageUrl && !value.coverImageUrl.startsWith('https://')) return 'Cover image URL must use HTTPS.';
  if (value.coverImageUrl && (!value.coverImageAltText || value.coverImageAltText.trim().length > 240)) return 'Cover image alt text required, maximum 240 characters.';
  return null;
}
