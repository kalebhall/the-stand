-- Reapply the worker identity provisioning function after fixture-safe metadata creation changes.
DO $$
DECLARE
        ...
        SELECT ward_row.id, worker_user_id, worker_role_id, 'System notifications worker'
         WHERE NOT EXISTS (
           SELECT 1 FROM public.ward_user_role wur
            WHERE wur.ward_id = ward_row.id AND wur.user_id = worker_user_id
              AND wur.role_id = worker_role_id AND wur.revoked_at IS NULL
         );
    END LOOP;
...
SET search_path TO pg_catalog, public SET row_security TO on
AS $$
DECLARE previous_ward text := current_setting('app.ward_id', true);
BEGIN
    IF EXISTS (
        SELECT 1 FROM public.user_account
    ...
    END IF;
    INSERT INTO public.user_account (id, email, display_name, password_hash, is_active)
    VALUES ('00000000-0000-0000-0000-000000000000'::uuid, 'notifications-worker@invalid.local', 'Notifications Worker', NULL, true)
    ON CONFLICT (id) DO UPDATE SET is_active = true, password_hash = NULL;
    INSERT INTO public.role (id, name, scope)
    ...
    INSERT INTO public.ward_user_role (ward_id, user_id, role_id, grant_reason)
    SELECT NEW.id, '00000000-0000-0000-0000-000000000000'::uuid,
           '00000000-0000-0000-0000-000000000001'::uuid, 'System notifications worker'
     WHERE NOT EXISTS (
       SELECT 1 FROM public.ward_user_role wur
