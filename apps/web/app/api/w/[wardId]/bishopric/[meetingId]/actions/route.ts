...
export async function POST(request: Request, context: { params: Promise<{ wardId: string; meetingId: string }> }) {
  ...
  if (!canManageMeetings({ roles: session.user.roles, activeWardId: session.activeWardId }, wardId) || !(await isWardModuleEnabled(wardId, session.user.id, 'bishopric'))) return NextResponse.json({ error: 'Forbidden', code: 'FORBIDDEN' }, { status: 403 });
