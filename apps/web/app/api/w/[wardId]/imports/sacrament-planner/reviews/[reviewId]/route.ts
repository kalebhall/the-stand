...
export async function PATCH(request: Request, context: { params: Promise<{ wardId: string; reviewId: string }> }) {
  ...
  if (action !== 'resolve' && action !== 'ignore') return NextResponse.json({ error: 'Action must be resolve or ignore', code: 'BAD_REQUEST' }, { status: 400 });
  if (action === 'resolve' && typeof body?.memberId !== 'string') return NextResponse.json({ error: 'A member is required to resolve a name', code: 'BAD_REQUEST' }, { status: 400 });

  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    await setDbContext(client, { userId: session.user.id, wardId });
    const review = await client.query('SELECT source_name FROM historical_import_name_review WHERE id = $1::uuid AND ward_id = $2::uuid AND status = \'OPEN\' LIMIT 1', [reviewId, wardId]);
    if (!review.rowCount) {
    ...
    if (action === 'ignore') {
      await client.query(`UPDATE historical_import_name_review SET status = 'IGNORED', updated_at = now() WHERE id = $1::uuid AND ward_id = $2::uuid`, [reviewId, wardId]);
    } else {
      const member = await client.query('SELECT full_name FROM member WHERE id = $1::uuid AND ward_id = $2::uuid LIMIT 1', [body?.memberId, wardId]);
      if (!member.rowCount) {
        await client.query('ROLLBACK');
      ...
      await client.query(`UPDATE historical_import_name_review SET status = 'RESOLVED', matched_member_id = $1::uuid, resolved_name = $2::text, updated_at = now() WHERE id = $3::uuid AND ward_id = $4::uuid`, [body?.memberId, member.rows[0].full_name, reviewId, wardId]);
    }
    await client.query(
