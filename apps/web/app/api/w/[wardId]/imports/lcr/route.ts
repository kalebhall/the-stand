...
async function runLcrImport(params: {
  wardId: string;
  userId: string;
  username: string;
  password: string;
...
}): Promise<LcrJobResult> {
  const imported = await importFromLcr({
  ...
  try {
    ...
    await setDbContext(client, { userId: params.userId, wardId: params.wardId });
    ...
    params.logger.error('Failed to process LCR import job', {
      wardId: params.wardId,
      userId: params.userId,
      commitRequested: params.commit,
      parsedMemberCount: imported.members.length,
      parsedCallingCount: imported.callings.length,
      error: message
    });
    throw new Error(`Failed to process LCR import: ${message}`);
  } finally {
    client.release();
  }
}
...
export async function POST(request: Request, context: { params: Promise<{ wardId: string }> }) {
  ...
  const job = enqueueLcrImportJob<LcrJobResult>({
    wardId,
