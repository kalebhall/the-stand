...
export async function POST(_: Request, context: { params: Promise<{ wardId: string; callingId: string }> }) {
  ...
  try {
    ...
    await recordAuditEvent(client, {
      ...
      callingStatus: CALLING_STATUS.SUSTAINED,
      changes: {
        status: { old: currentStatus, new: CALLING_STATUS.SUSTAINED }
      },
      details: {
    ...
    const eventOutboxId = await insertNotificationOutboxEvent(client, {
      ...
      eventType: 'CALLING_SUSTAINED',
      payload: { actorUserId: session.user.id, callingAssignmentId: callingId, status: CALLING_STATUS.SUSTAINED }
    });
