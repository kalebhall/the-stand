...
export async function PUT(request: Request, context: { params: Promise<{ wardId: string; noteId: string }> }) {
  ...
  try {
    ...
    const existing = await client.query('SELECT id, visibility, created_by_user_id FROM internal_note WHERE id = $1::uuid AND ward_id = $2::uuid LIMIT 1', [noteId, wardId]);
    if (!existing.rowCount) {
      await client.query('ROLLBACK');
      return NextResponse.json({ error: 'Note not found', code: 'NOT_FOUND' }, { status: 404 });
    }
    const note = existing.rows[0] as { visibility: string; created_by_user_id: string };
