...
export async function PUT(request: Request, context: { params: Promise<{ wardId: string; noteId: string }> }) {
  ...
  try {
    ...
    const existing = await client.query('SELECT id, visibility, note_text, program_item_id, created_by_user_id FROM internal_note WHERE id = $1::uuid AND ward_id = $2::uuid LIMIT 1', [noteId, wardId]);
    ...
    const mayEdit = note.created_by_user_id === session.user.id || (note.visibility === 'LEADERSHIP' && canUseInternalNotes({ roles: session.user.roles, activeWardId: session.activeWardId }, wardId));
    if (!mayEdit) {
      await client.query('ROLLBACK');
      return NextResponse.json({ error: 'Forbidden', code: 'FORBIDDEN' }, { status: 403 });
    }
    ...
    if (note.visibility === 'PUBLIC' && note.program_item_id) {
      await client.query('UPDATE meeting_program_item SET program_notes = $1::text WHERE id = $2::uuid AND ward_id = $3::uuid', [parsed.data.noteText, note.program_item_id, wardId]);
    }
