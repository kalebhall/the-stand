...
describe('internal notes routes', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    authMock.mockResolvedValue({ user: { id: userId, email: 'leader@example.com', name: 'Leader', roles: ['STAND_ADMIN'] }, activeWardId: wardId });
  ...
  it('creates a meeting note only after verifying ward-scoped target', async () => {
    ...
    const response = await POST(request({ target: { type: 'MEETING', meetingId }, visibility: 'PRIVATE', noteText: '  Follow up  ' }), { params: Promise.resolve({ wardId }) });

    expect(response.status).toBe(201);
    expect(await response.json()).toEqual({ id: noteId, createdAt: '2026-08-28T00:00:00.000Z' });
    expect(queryMock).toHaveBeenNthCalledWith(2, expect.stringContaining('FROM meeting WHERE id = $1::uuid AND ward_id = $2::uuid'), [meetingId, wardId]);
    expect(queryMock).toHaveBeenNthCalledWith(3, expect.stringContaining('program_item_id'), [wardId, null, meetingId, null, 'PRIVATE', 'Follow up', userId]);
    expect(recordAuditEventMock).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ action: 'INTERNAL_NOTE_CREATED', entityId: noteId }));
    expect(queryMock).toHaveBeenLastCalledWith('COMMIT');
  });

  it('rejects invalid note payload before database access', async () => {
    const response = await POST(request({ target: { type: 'MEETING', meetingId }, visibility: 'PRIVATE', noteText: '   ' }), { params: Promise.resolve({ wardId }) });
  ...
  it('updates an existing note and records audit activity', async () => {
    ...
    const response = await PUT(
      new Request('http://localhost', { method: 'PUT', body: JSON.stringify({ noteText: 'Updated note' }), headers: { 'content-type': 'application/json' } }),
      { params: Promise.resolve({ wardId, noteId }) }
    );

    expect(response.status).toBe(200);
    expect(await response.json()).toEqual({ success: true });
    expect(queryMock).toHaveBeenNthCalledWith(3, expect.stringContaining('UPDATE internal_note SET note_text = $1::text'), ['Updated note', noteId, wardId]);
    expect(recordAuditEventMock).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ action: 'INTERNAL_NOTE_UPDATED', entityId: noteId }));
    expect(queryMock).toHaveBeenLastCalledWith('COMMIT');
  });
});
