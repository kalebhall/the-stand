...
export async function PATCH(request: Request, context: { params: Promise<{ wardId: string; meetingId: string; actionId: string }> }) {
  ...
  if (status !== 'announced' && status !== 'completed') return NextResponse.json({ error: 'Invalid status.', code: 'INVALID_INPUT' }, { status: 400 });

  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    await setDbContext(client, { userId: session.user.id, wardId });
    const result = status === 'announced'
      ? await client.query(
    ...
    await client.query(
      ...
      [wardId, session.user.id, status === 'announced' ? 'MEMBERSHIP_ORDINANCE_ANNOUNCED' : 'MEMBERSHIP_ORDINANCE_COMPLETED', meetingId, actionId]
    );
...
export async function DELETE(_request: Request, context: { params: Promise<{ wardId: string; meetingId: string; actionId: string }> }) {
  ...
  try {
    await client.query('BEGIN');
    await setDbContext(client, { userId: session.user.id, wardId });
    const result = await client.query('DELETE FROM meeting_membership_ordinance WHERE id = $1::uuid AND meeting_id = $2::uuid AND ward_id = $3::uuid RETURNING id', [actionId, meetingId, wardId]);
    if (!result.rowCount) { await client.query('ROLLBACK'); return NextResponse.json({ error: 'Action not found.', code: 'NOT_FOUND' }, { status: 404 }); }
    await client.query('COMMIT');
