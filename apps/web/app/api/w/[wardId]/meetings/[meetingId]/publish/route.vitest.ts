...
const { authMock, canManageMeetingsMock, setDbContextMock, enqueueOutboxNotificationJobMock, queryMock, releaseMock, connectMock } =
  vi.hoisted(() => ({
    authMock: vi.fn(),
...
describe('POST /api/w/[wardId]/meetings/[meetingId]/publish', () => {
  ...
  it('creates a new immutable render version and marks meeting as published', async () => {
    ...
    expect(queryMock).toHaveBeenCalledWith(
      ...
      expect.arrayContaining(['ward-1', 'user-1', 'MEETING_REPUBLISHED'])
    );
