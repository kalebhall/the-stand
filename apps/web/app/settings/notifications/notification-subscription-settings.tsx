    ...
    <div className="space-y-8">
      {!hasUsableEmail && (
        <p className="rounded border border-amber-400 bg-amber-50 p-3 text-sm text-amber-950" role="note">
          Email notifications are unavailable because this account has no usable email address.
        </p>
      )}
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <span className="text-sm text-muted-foreground" role="status">
          {status === 'saving' ? 'Saving…' : status === 'saved' ? 'Saved' : (error ?? 'Changes are not saved yet.')}
        </span>
      </div>
      {groups.map((category) => (
                ...
                >
                  ...
                  {(['IN_APP', 'EMAIL'] as const).map((channel) => (
                    <label key={channel} className="flex items-center gap-2 text-sm">
                      ...
                      />{' '}
                      <span className="sm:hidden">{CHANNEL_LABELS[channel]}</span>
                    </label>
                  ))}
                </div>
              ))}
