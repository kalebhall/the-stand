...
const subscriptions = [
  {
    eventType: 'CALLING_SUGGESTED' as const,
    category: 'CALLINGS' as const,
  ...
  it('saves both channel values as one batch and shows saved state', async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ subscriptions, emailPreference: { frequency: 'IMMEDIATE', timezone: 'UTC' } }), { status: 200 })
      )
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ subscriptions, emailPreference: { frequency: 'DAILY', timezone: 'UTC' } }), { status: 200 })
      );
    ...
    const body = JSON.parse((request[1] as RequestInit).body as string) as {
      subscriptions: Array<{ eventType: string; channel: string; enabled: boolean }>;
      emailPreference: { frequency: string; timezone: string };
    };
    ...
    expect(body.emailPreference).toEqual({ frequency: 'DAILY', timezone: 'UTC' });
  });
});
