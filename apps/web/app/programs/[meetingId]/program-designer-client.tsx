...
export function ProgramDesignerClient({ wardId, meetingId }: Props) {
  ...
  function updateSelectedBlock(mutator: (block: DocumentBlock) => DocumentBlock) {
    ...
    if (advancedEditing) {
      ...
      for (const page of next.pages) for (const region of page.regions) {
        ...
        if (index >= 0) { region.blocks[index] = { ...region.blocks[index], ...mutator(region.blocks[index]) }; applyAdvanced(next); return; }
      }
      return;
    }
  ...
  function updateAdvancedVisibility(value: DocumentBlock['visibility']) {
    const current = currentAdvanced();
    if (current && selectedBlockId) applyAdvanced(setAdvancedBlockVisibility(current, selectedBlockId, value));
  }

  function moveSelectedBlock(delta: -1 | 1) {
    if (advancedEditing) { const current = currentAdvanced(); if (current && selectedBlockId) applyAdvanced(reorderBlock(current, selectedBlockId, delta)); return; }
  ...
  function updateTheme(theme: Partial<DocumentLayout['theme']>) {
    if (advancedEditing) { const current = currentAdvanced(); if (current) applyAdvanced({ ...current, theme: { ...current.theme, ...theme } }); return; }
