...
export function SacramentPlannerImportClient({ wardId }: { wardId: string }) {
  ...
  useEffect(() => { void loadReviews(); }, [wardId]);
      ...
      {reviews.length ? (
        <section className="space-y-4 rounded-lg border bg-card p-5 shadow-sm">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Choose the correct ward member to replace the source text across imported program history. If it is a guest or you are unsure, ignore it and leave the original text unchanged.</p>
          </div>
            ...
            {reviews.map((review) => (
              <div key={review.id} className="rounded-md border p-3">
                <div className="flex flex-wrap items-center justify-between gap-2 text-sm">
                  ...
                  <span className="text-muted-foreground">{review.occurrence_count} occurrence{review.occurrence_count === 1 ? '' : 's'} · {formatHistoricalDate(review.first_seen_date)} to {formatHistoricalDate(review.last_seen_date)}</span>
                </div>
                <div className="mt-3 flex flex-wrap gap-2">
                  <select aria-label={`Match ${review.source_name} to a member`} value={selections[review.id] ?? ''} onChange={(event) => setSelections((current) => ({ ...current, [review.id]: event.target.value }))} className="min-w-64 rounded-md border bg-background px-3 py-2 text-sm">
                    <option value="">Choose a member...</option>
                    {members.map((member) => <option key={member.id} value={member.id}>{member.full_name}</option>)}
                  </select>
                  <Button type="button" size="sm" onClick={() => void reviewName(review.id, 'resolve')} disabled={busy || !selections[review.id]}>Use selected member</Button>
                  <Button type="button" size="sm" variant="outline" onClick={() => void reviewName(review.id, 'ignore')} disabled={busy}>Keep source text</Button>
                </div>
              </div>
            ))}
          </div>
