...
export default async function BishopricPage() {
  ...
  return <main className="mx-auto w-full max-w-6xl space-y-6 p-4 sm:p-6 lg:p-8">
    <section className="space-y-2"><div className="flex flex-wrap items-start justify-between gap-3"><div><h1 className="text-2xl font-semibold tracking-tight">Bishopric workspace</h1><p className="mt-1 text-sm text-muted-foreground">Private bishopric agenda, decisions, assignments, and due dates. Not part of public meeting programs.</p></div><a href="/dashboard" className={cn(buttonVariants({ variant: 'outline' }))}>Dashboard</a></div><p className="text-xs text-muted-foreground">Visibility is private and ward-scoped. Do not store confidential counseling details here.</p></section>
    <BishopricWorkspaceClient wardId={session.activeWardId} initialMeetings={meetings} initialActions={actions} />
  </main>;
}
