...
**Implementation status:** Added isolated restore smoke validation with optional checksum verification, migration-state validation, core schema checks, representative row count, explicit temporary-database cleanup, and secret-free result metrics. Added local backup freshness/integrity monitoring, an encrypted restic off-host replication runner, and repository systemd service/timer units. Production replication uses the Proxmox `ProxBackup` NFS storage through a dedicated restricted SFTP key; the restic repository is client-side encrypted and pruned independently of local artifacts. Documented 14-day local retention, off-site retention, RPO/RTO assumptions, quarterly drill procedure, and operational ownership. Real deployment drill completed 2026-09-06 against latest backup `the_stand_20260905_072725.sql.gz`: restore succeeded, temporary database count returned 0 after cleanup, `/health` returned `{"status":"ok","db":"connected"}`, and `the-stand` remained active. Checksum sidecar verification remains conditional on sidecar presence.
